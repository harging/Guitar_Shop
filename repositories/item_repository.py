...
def update(item):
    sql = "UPDATE items SET (item_id, name, brand, description, buying_cost, selling_price) = (%s, %s, %s, %s, %s, %s) WHERE id = %s"
    values = [item.id, item.name, item.brand.id, item.description, item.buying_cost, item.selling_price]
    print(values)
    run_sql(sql, values)
